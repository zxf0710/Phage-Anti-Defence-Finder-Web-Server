<template>
  <div class="app-container" style="font-family: 'Times New Roman'">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="参考文献" prop="pubmedId">
        <el-input
          v-model="queryParams.pubmedId"
          placeholder="请输入参考文献"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:vdetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:vdetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:vdetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="vdetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column v-if="false" label="已验证详情id" align="center" prop="verifiedDetailId" />
      ...
      <el-table-column label="参考文献" align="center" prop="pubmedId" />
...
<!--      <el-table-column label="备用字段5" align="center" prop="spare5" />-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="参考文献" prop="pubmedId">
          <el-input v-model="form.pubmedId" placeholder="请输入参考文献" />
        </el-form-item>
<!--        <el-form-item label="备用字段1" prop="spare1">-->
...
<script>
import { listVdetail, getVdetail, delVdetail, addVdetail, updateVdetail } from "@/api/system/vdetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listVdetail(this.queryParams).then(response => {
        this.vdetailList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.verifiedDetailId)
    ...
    handleUpdate(row) {
      this.reset();
      const verifiedDetailId = row.verifiedDetailId || this.ids
      getVdetail(verifiedDetailId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.verifiedDetailId != null) {
            updateVdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addVdetail(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const verifiedDetailIds = row.verifiedDetailId || this.ids;
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + verifiedDetailIds + '"的数据项？').then(function() {
        return delVdetail(verifiedDetailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/vdetail/export', {
        ...this.queryParams
      }, `vdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
