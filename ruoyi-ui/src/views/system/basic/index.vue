<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="basicList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="basicId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="噬菌体" prop="phage">
          <el-input v-model="form.phage" type="textarea" placeholder="请输入内容" />
        </el-form-item>
...
<script>
import { listBasic, getBasic, delBasic, addBasic, updateBasic } from "@/api/system/basic";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.basicId)
    ...
    handleUpdate(row) {
      this.reset();
      const basicId = row.basicId || this.ids
      getBasic(basicId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const basicIds = row.basicId || this.ids;
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + basicIds + '"的数据项？').then(function() {
        return delBasic(basicIds);
      }).then(() => {
        this.getList();
