    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:verified:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:verified:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:verified:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:verified:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="verifiedList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column v-if="false" label="已验证id" align="center" prop="verifiedId" />
...
<script>
import { listVerified, getVerified, delVerified, addVerified, updateVerified } from "@/api/system/verified";
  ...
  methods: {
    ...
    getList() {
      ...
      listVerified(this.queryParams).then(response => {
        this.verifiedList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.verifiedId)
    ...
    handleUpdate(row) {
      this.reset();
      const verifiedId = row.verifiedId || this.ids
      getVerified(verifiedId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.verifiedId != null) {
            updateVerified(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addVerified(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const verifiedIds = row.verifiedId || this.ids;
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + verifiedIds + '"的数据项？').then(function() {
        return delVerified(verifiedIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/verified/export', {
        ...this.queryParams
      }, `verified_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
