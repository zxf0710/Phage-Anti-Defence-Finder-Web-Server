<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="系族" prop="systemFamily">
        <el-input
      ...
      <el-form-item label="蛋白序列" prop="sequence">
        <el-input
      ...
      <el-form-item label="hmmEValue" prop="hmmEValue">
        <el-input
          v-model="queryParams.hmmEValue"
          placeholder="请输入hmmEValue"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="hmmScore" prop="hmmScore">
        <el-input
          v-model="queryParams.hmmScore"
          placeholder="请输入hmmScore"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:pdetail:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:pdetail:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:pdetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="pdetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column v-if="false" label="待预测详情id" align="center" prop="predictedDetailId" />
      <el-table-column label="系族" align="center" prop="systemFamily" />
      ...
      <el-table-column label="蛋白序列" align="center" prop="sequence" />
      <el-table-column label="长度" align="center" prop="length" />
      <el-table-column label="hmmEValue" align="center" prop="hmmEValue" />
      <el-table-column label="hmmScore" align="center" prop="hmmScore" />
...
<!--      <el-table-column label="备用字段5" align="center" prop="spare5" />-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="系族" prop="systemFamily">
          <el-input v-model="form.systemFamily" placeholder="请输入系族" />
        </el-form-item>
        <el-form-item label="ID" prop="proteinIds">
        ...
        <el-form-item label="蛋白序列" prop="sequence">
          <el-input v-model="form.sequence" placeholder="请输入蛋白序列" />
        </el-form-item>
        <el-form-item label="长度" prop="length">
          <el-input v-model="form.length" placeholder="请输入长度" />
        </el-form-item>
        <el-form-item label="hmmEValue" prop="hmmEValue">
          <el-input v-model="form.hmmEValue" placeholder="请输入hmmEValue" />
        </el-form-item>
        <el-form-item label="hmmScore" prop="hmmScore">
          <el-input v-model="form.hmmScore" placeholder="请输入hmmScore" />
        </el-form-item>
<!--        <el-form-item label="备用字段1" prop="spare1">-->
...
<script>
import { listPdetail, getPdetail, delPdetail, addPdetail, updatePdetail } from "@/api/system/pdetail";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPdetail(this.queryParams).then(response => {
        this.pdetailList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.predictedDetailId)
    ...
    handleUpdate(row) {
      this.reset();
      const predictedDetailId = row.predictedDetailId || this.ids
      getPdetail(predictedDetailId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.predictedDetailId != null) {
            updatePdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPdetail(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const predictedDetailIds = row.predictedDetailId || this.ids;
      this.$modal.confirm('是否确认删除【请填写功能名称】编号为"' + predictedDetailIds + '"的数据项？').then(function() {
        return delPdetail(predictedDetailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/pdetail/export', {
        ...this.queryParams
      }, `pdetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
